...
///
/// Archived: preserved for reference — replaced by `finalize_cascading`.
#[allow(dead_code)]
fn _finalize_siblings_archived(schemas: &mut Vec<TableSchema>, threshold: usize, min_jaccard: f64) {
...
// ---------------------------------------------------------------------------
// Cascading sibling detection (replaces finalize_siblings)
// ---------------------------------------------------------------------------

/// BFS top-down sibling fusion with child-compatibility gate and cascaded merging.
///
/// Wave 0 — same detection logic as `_finalize_siblings_archived` but with two additions:
...
fn finalize_cascading(schemas: &mut Vec<TableSchema>, threshold: usize, min_jaccard: f64) {
...
/// Run wave 0 of sibling detection: same logic as `_finalize_siblings_archived` but with
