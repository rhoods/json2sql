...
// ---------------------------------------------------------------------------
// Test — Override de type valide : score FLOAT → TEXT
//
// users.jsonl contient `score` comme float (9.5, 7.2, 8.8).
// Pass 1 l'infère DoublePrecision. override_score.toml force `score = "TEXT"`
// pour la table "people". Après apply_overrides + Pass 2, la colonne score
// doit être de type `text` dans pg_catalog avec les valeurs exactes stockées.
// ---------------------------------------------------------------------------
#[tokio::test]
...
// ---------------------------------------------------------------------------
// Test — Override invalide : score → INTEGER (floats → anomalies),
...
async fn test_override_bad() {
    ...
    // row_count détecte une divergence silencieuse entre pipeline et réalité.
    assert_eq!(*p2.rows_per_table.get("people").unwrap(), 3);
